...
export default function App() {
  ...
  const handleRegister = async (payload) => {
    try {
      ...
      if (res.ok) {
        ...
        toast.success(
          `Account registered! Welcome ${data.user.name}.`
        );
  ...
  const handleCreateComplaint = async (complaintData) => {
    try {
      ...
      if (res.ok) {
        ...
        toast.error(
          errorData.error || "Failed to file grievance."
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleEscalateManual = async (id) => {
    try {
      ...
      if (res.ok) {
        toast.success(
          "Ticket manually escalated to next Higher Authority tier!"
        );
        ...
        toast.error(
          data.error || "Failed to escalate ticket."
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleOverrideComplaint = async (
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/complaints/${id}/override`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({
  ...
  const handleRateResolution = async (
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/complaints/${id}/rate`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({
      ...
      if (res.ok) {
        toast.success(
          `Thank you! Submitted ${rating}-star resolution feedback.`
        );
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
              ...
              {loadingAudit ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-5 items-start">
                    ...
                    <div className="sm:col-span-8 space-y-2">
                      ...
                      <div className="text-[10px] text-slate-400 font-mono">
                        Loc: {auditItem.location}
                      </div>
                    </div>
                  </div>

                  {/* Rating / Escalation Block */}
                  {user &&
                    user.role === "citizen" &&
                    (auditItem.citizen?._id === user._id ||
                      auditItem.citizen === user._id) && (
                      <div className="p-4 bg-sky-50 border border-sky-100 rounded-xl space-y-3">
                        ...
                        {/* State level tier notification */}
                        {auditItem.tier === "state" && (
                          <div className="p-3 bg-indigo-50/80 border border-indigo-200 rounded-xl text-indigo-900 text-xs">
                            <span className="font-bold block mb-0.5 flex items-center gap-1">
                              <Shield
                                size={12}
                                className="text-indigo-600"
                              />
                              State Level (Highest Authority Tier)
                            </span>

                            This grievance is under top-tier state jurisdiction. No further escalation can occur.
                          </div>
                        )}

                        {/* Escalate option */}
                        {auditItem.status !== "resolved" &&
                          auditItem.tier !== "state" && (
                              ...
                              {!showEscalateConfirm ? (
                                <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center w-full">
                                  ...
                                  <p className="text-[11px] text-sky-850">
                                    Not satisfied with response? Escalate manually to the next administrative tier level (
                                    {auditItem.tier === "local"
                                      ? "District"
                                      : "State"}
                                    ).
                                  </p>
                                ...
                                <div className="space-y-2.5 w-full">
                                  ...
                                  <p className="text-xs font-bold text-slate-800">
                                    Are you sure you want to escalate this complaint to the{" "}
                                    {auditItem.tier ===
                                    "local"
                                      ? "District"
                                      : "State"}{" "}
                        ...
                        {/* Rating input */}
                        {auditItem.status === "resolved" &&
                          auditItem.satisfactionRating ===
                            undefined && (
                            <div className="flex flex-col gap-2">

                              <label className="text-xs font-bold text-sky-900">
                                Rate Citizen Satisfaction
                              </label>
                  ...
                  <div className="space-y-4">
                    ...
                    <h4 className="text-xs font-bold text-slate-800 uppercase tracking-widest font-mono">
                      Official Transparency Timeline
                    </h4>

                    <div className="relative pl-6 border-l border-slate-200 space-y-6">

                      {auditItem.statusHistory &&
                        auditItem.statusHistory.map(
                          (history, idx) => (
                            <div
                              key={idx}
                              className="relative"
                              ...
                              <div className="space-y-1">
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-xs font-bold text-slate-800">
                                    Status updated:{" "}
                                    <span className="uppercase text-sky-600">
                                      {history.status}
                                    </span>
                                  </span>
                ...
                <div className="text-center py-12 text-slate-400 text-sm font-semibold">
                  Unable to fetch grievance trail details.
                </div>
              )}
            </div>
