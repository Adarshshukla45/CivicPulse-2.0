      ...
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200 grid grid-cols-1 md:grid-cols-12">
        ...
        <div className="p-8 sm:p-12 md:col-span-5 bg-slate-900 text-white flex flex-col justify-between border-t md:border-t-0 md:border-l border-slate-800">
          <div className="space-y-6">
            ...
            <div className="space-y-2.5 pt-2 max-h-[460px] overflow-y-auto pr-1">
              {/* Citizen fast login */}
              <button
                onClick={() => triggerQuickLogin("aarav@gmail.com", "citizen123")}
                disabled={submitting}
                className="w-full bg-slate-800/80 hover:bg-slate-800 p-3 rounded-xl border border-slate-750 text-left transition hover:border-blue-500/50 flex items-center justify-between group cursor-pointer"
              >
                <div>
                  <div className="flex items-center gap-2">
                    <span className="p-1 rounded-md bg-blue-500/10 text-blue-400"><User size={12} /></span>
                    <span className="text-[11px] font-bold text-blue-400">Citizen Profile</span>
                  </div>
                  <h3 className="text-xs font-semibold mt-0.5 group-hover:text-blue-300 transition">Aarav Sharma</h3>
              ...
              <button
                ...
                disabled={submitting}
                className="w-full bg-slate-800/80 hover:bg-slate-800 p-3 rounded-xl border border-slate-750 text-left transition hover:border-red-500/50 flex items-center justify-between group cursor-pointer"
              >
                <div>
                  <div className="flex items-center gap-2">
                    <span className="p-1 rounded-md bg-red-500/10 text-red-400"><Shield size={12} /></span>
                    <span className="text-[11px] font-bold text-red-400">Super Admin (All Departments)</span>
                  </div>
                  <h3 className="text-xs font-semibold mt-0.5 group-hover:text-red-300 transition">Super Admin HQ</h3>
              ...
              <div className="pt-2 pb-1">
                <span className="text-[10px] font-mono uppercase text-slate-400 font-bold block">Department Officers:</span>
              </div>

              {[
                { name: "Rajesh (Roads)", email: "roads@civicpulse.org", dept: "Roads Officer" },
                { name: "Suresh (Water)", email: "water@civicpulse.org", dept: "Water Officer" },
                { name: "Vikram (Power)", email: "power@civicpulse.org", dept: "Power Officer" },
                { name: "Anita (Sanitation)", email: "sanitation@civicpulse.org", dept: "Sanitation Officer" },
                { name: "Dr. Neha (Health)", email: "health@civicpulse.org", dept: "Health Officer" },
              ].map(officer => (
                <button
                  key={officer.email}
                  onClick={() => triggerQuickLogin(officer.email, "admin123")}
                  disabled={submitting}
                  className="w-full bg-slate-800/60 hover:bg-slate-800 p-2.5 rounded-xl border border-slate-750 text-left transition hover:border-amber-500/50 flex items-center justify-between group cursor-pointer"
                >
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="p-0.5 rounded bg-amber-500/10 text-amber-400"><Cpu size={12} /></span>
                      <span className="text-[10px] font-bold text-amber-400">{officer.dept}</span>
                    </div>
                    <h3 className="text-xs font-semibold mt-0.5 group-hover:text-amber-300 transition">{officer.name}</h3>
                    <span className="text-[9px] text-slate-500 font-mono">{officer.email}</span>
                  </div>
