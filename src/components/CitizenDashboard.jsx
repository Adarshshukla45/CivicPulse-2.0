...
export default function CitizenDashboard({
  ...
  const handleFormSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const matched = departments ? departments.find(d => d.name.toLowerCase() === category.toLowerCase() || d._id === category) : null;
      const success = await onCreateComplaint({
        title,
        description,
        category,
        departmentId: matched ? matched._id : undefined,
        location,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-sky-500 font-medium text-slate-800"
                >
