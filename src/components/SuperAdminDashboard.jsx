...
export default function SuperAdminDashboard({
  ...
  const [allComplaintsFilterDept, setAllComplaintsFilterDept] = useState("all");
  const [allComplaintsFilterStatus, setAllComplaintsFilterStatus] = useState("all");
  const [allComplaintsSearch, setAllComplaintsSearch] = useState("");
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex border-b border-slate-200 gap-6 overflow-x-auto">
        {[
          ...
          { id: "all_complaints", label: `All Grievances (${allComplaints ? allComplaints.length : 0})` },
          { id: "slas", label: "SLA Config" },
          { id: "users", label: "Users & Roles" },
          { id: "overrides", label: "Escalated Overrides" },
        ].map(t => (
          <button
            key={t.id}
            onClick={() => setActiveTab(t.id)}
            className={`pb-2.5 text-sm font-semibold transition border-b-2 uppercase tracking-wide whitespace-nowrap cursor-pointer ${
      ...
      {activeTab === "all_complaints" && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden space-y-4">
          <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row justify-between md:items-center gap-4">
            <div>
              <h2 className="text-base font-bold text-slate-900 font-display">Municipal Master Grievance Log</h2>
              <p className="text-xs text-slate-400">Complete multi-department visibility across Roads, Water, Power, Sanitation, Health, and other domains.</p>
            </div>
            <div className="flex items-center gap-2">
              <input
                type="text"
                placeholder="Search title, citizen, location..."
                value={allComplaintsSearch}
                onChange={e => setAllComplaintsSearch(e.target.value)}
                className="px-3 py-1.5 border border-slate-200 rounded-xl text-xs bg-slate-50 focus:outline-none focus:ring-1 focus:ring-sky-500 w-64"
              />
            </div>
          </div>

          {/* Filters Bar */}
          <div className="px-5 flex flex-wrap gap-4 items-center justify-between pb-3 border-b border-slate-100">
            {/* Department Filter */}
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[10px] font-mono font-bold uppercase text-slate-400 mr-1">Dept:</span>
              {["all", ...(departments ? departments.map(d => d.name.toLowerCase()) : ["roads", "water", "power", "sanitation", "health"])].map(dept => (
                <button
                  key={dept}
                  onClick={() => setAllComplaintsFilterDept(dept)}
                  className={`px-2.5 py-1 text-xs font-semibold rounded-lg border transition uppercase cursor-pointer ${
                    allComplaintsFilterDept === dept
            ...
            {/* Status Filter */}
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[10px] font-mono font-bold uppercase text-slate-400 mr-1">Status:</span>
              {["all", "pending", "in_review", "escalated", "resolved", "rejected"].map(st => (
                <button
                  key={st}
                  onClick={() => setAllComplaintsFilterStatus(st)}
                  className={`px-2.5 py-1 text-xs font-semibold rounded-lg border transition uppercase cursor-pointer ${
                    allComplaintsFilterStatus === st
          ...
          <div className="divide-y divide-slate-150">
            {(() => {
              const list = (allComplaints || []).filter(c => {
                const cDeptName = (typeof c.department === "object" ? c.department?.name : c.category || "").toLowerCase();
                const matchDept = allComplaintsFilterDept === "all" || cDeptName === allComplaintsFilterDept || cDeptName.includes(allComplaintsFilterDept);
                const matchStatus = allComplaintsFilterStatus === "all" || c.status === allComplaintsFilterStatus;
                const q = allComplaintsSearch.toLowerCase().trim();
                const matchSearch = !q || (
              ...
              if (list.length === 0) {
                return (
                  <div className="text-center py-16 text-slate-400 text-sm font-mono">
                    No complaints match current department/status filter.
                  </div>
                );
              }
                  ...
                  <div key={c._id} className="p-5 flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:bg-slate-50/50">
                    <div className="space-y-1 max-w-xl">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-[10px] font-mono bg-slate-200 text-slate-800 px-2 py-0.5 rounded font-bold uppercase">
                          {deptName}
                        </span>
                        <span className="text-[10px] font-mono bg-slate-800 text-white px-1.5 py-0.5 rounded uppercase font-semibold">
                          {c.tier} Tier
                        </span>
                        <span className={`text-[10px] font-mono font-bold uppercase px-2 py-0.5 rounded ${
                          c.status === "resolved" ? "bg-emerald-100 text-emerald-800" :
                          c.status === "rejected" ? "bg-rose-100 text-rose-800" :
                          c.status === "escalated" ? "bg-amber-100 text-amber-800 animate-pulse" :
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-2 hover:bg-slate-100 border border-slate-200 text-slate-600 rounded-lg text-xs font-semibold flex items-center gap-0.5 cursor-pointer"
                      >
                        <Eye size={12} /> Audit Trail
                      </button>
                      <button
                        onClick={() => {
                          const remarkStr = prompt(`Enter resolution/action remark for "${c.title}":`);
                          if (remarkStr) {
